...
def timezone_kb() -> ReplyKeyboardMarkup:
    offsets = [f"{i:+d}" for i in range(-12, 12)]
...
async def settings_command(message: Message, state: FSMContext) -> None:
    await message.answer("✏️Выберите настройку", reply_markup=settings_menu_kb())
...
async def choose_option(message: Message, state: FSMContext) -> None:
    ...
    if "час" in text:
        await message.answer("✏️ Выберите часовой пояс", reply_markup=timezone_kb())
        await state.set_state(Setup.waiting_for_timezone)
        return
    if "квадрат" in text:
        await message.answer("✏️ Выберите часть", reply_markup=quadrant_kb())
    ...
    if "назад" in text:
        ...
        return
    await message.answer("✏️ Выберите настройку", reply_markup=settings_menu_kb())
...
async def choose_quadrant(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("✏️ Выберите цвет", reply_markup=colors_kb())
...
async def set_color(message: Message, state: FSMContext) -> None:
    ...
    if not re.match(COLOR_PATTERN, color):
        await message.answer("✍️ Введите цвет в HEX, например #ff8c00:", reply_markup=colors_kb())
        return
